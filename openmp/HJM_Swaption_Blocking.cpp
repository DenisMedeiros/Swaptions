  ...
  
{
  ...
  for (l=0;l<=lTrials-1;l+=BLOCKSIZE) {
    ...
    /* GC: 51% of the time goes here */
      iSuccess = HJM_SimPath_Forward_Blocking(ppdHJMPath, iN, iFactors, dYears, 
        pdForward, pdTotalDrift,ppdFactors, &iRndSeed, BLOCKSIZE); 
	...
	for(b=0;b<=BLOCKSIZE-1;b++){
      ...
      iSuccess = Discount_Factors_Blocking(pdPayoffDiscountFactors, iN, 
        dYears, pdDiscountingRatePath, BLOCKSIZE); 

     if (iSuccess!=1) return iSuccess;
   

     // Continua abaixo com mais cálculos e possibilidades de 'return'
