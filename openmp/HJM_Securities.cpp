...
/* Nossa função paralelizada. */
void custom()
{
  # pragma omp parallel for num_threads(nThreads) default(none) \
    shared(swaptions, nSwaptions, swaption_seed, NUM_TRIALS) schedule(runtime) 
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	__parsec_roi_end();
        ...
        fprintf(stderr,"Swaption %d: [SwaptionPrice: %.10lf StdError: %.10lf] \n", 
                   i, swaptions[nSwaptions-1].dSimSwaptionMeanPrice, swaptions[nSwaptions-1].dSimSwaptionStdError);
	  ...
	  free_dmatrix(swaptions[i].ppdFactors, 0, swaptions[i].iFactors-1, 0, swaptions[i].iN-2);
         ...
         fprintf(stderr, "Tempo total de execução: %lf s\n", total);
